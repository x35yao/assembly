...
def homogeneous_transform(R,vect):
...
def inverse_homogeneous_transform(H):
    ...
    R = R.T
    origin = -R.dot(origin)
    return homogeneous_transform(R,list(origin.flatten()))
...
def get_HT_objs_in_base(obj_traj_in_base, templates_in_base, HT_templates_in_base, window_size = 5, markers_average = False):
    '''
    ...
    for individual in individuals:
        if not markers_average:
            ...
            template_to_obj_in_base, dist = match_template_to_obj(obj_template_in_base, df_individual, window_size = window_size)
